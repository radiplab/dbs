<?php
	// Test URL:
	// http://localhost:1024/dwv-simplistic/viewer-wrapper.php?casefolder=0000539
?>
<!DOCTYPE html>
<html>
<head>
...
function onLoad() {	
	...
	if (Object.keys(sequences).length > 1) {
		...
		var center = parseInt(document.documentElement.clientWidth / 2);
		var sequencesWidth = sequencesContainer.clientWidth;
...
function displayMessage(message_text) {
	...
	if (message_text != '') {
		...
		// Position it
		var top = '5px';
		pageHeaderHeight = document.getElementById('pageHeader').clientHeight
		if (pageHeaderHeight != 0) {
			top = parseInt((pageHeaderHeight/2) - (messageDiv.clientHeight()/2)) + 'px';
		}
		var left = parseInt(document.documentElement.clientWidth - messageDiv.clientWidth() - 10) + 'px';
...
function preloadSequences() {
	...
	for (var i = 0; i < numSequences; i++) {
		...
		if (numSequences == 1) {
			...
			var iframe_src = "viewer.html?input=dicom%2F" + encodeURI(casefolder) + "%2F" + encodeURI(filename);
			...
			iframe.src = "viewer.html?input=dicom%2F" + encodeURI(casefolder) + "%2F" + encodeURI(filename);
		}
	}
	resize();
}
...
function resize() {
	...
	for (var i = 0; i < numSequences; i++) {
		...
		var iframeHeight = window.innerHeight - $(pageHeader).height();
